...
public:
	enum type_e : u16 {
		STRUCT=0, // node contains other nodes (whose names must be unique in this collection)
		ARRAY, // node contains array of nodes of same name

		LOADED, // nodes were generated by loading from file, values are strings (str member of union), will get interpreted as proper typed Values during next pass
	...
	static bool union_str_active (type_e t) { return t == STRING || t == LOADED; } // which types use str member of union

private:
	type_e	type;
	mode_e	mode;

	type_e get_type () { return type; }
...
public:
	...
	void assign (void* val, Value::type_e new_type, Value::mode_e new_mode=Value::DIMENSIONLESS) {
	...
	static bool parse (std::string const& s, std::string* out) {
		*out = s;
		return true;
	}

	template <typename T, typename F>
	static bool parse_vec (std::string const& s, T* out, int comp_count, F parse_scalar) {
		T buf[4];
	...
	static bool parse (std::string const& s, int* out, int comp_count, mode_e mode) {
		return parse_vec(s, out, comp_count, n_parse::signed_int);
	}
	static bool parse (std::string const& s, flt* out, int comp_count, mode_e mode) {
		return parse_vec(s, out, comp_count, [&] (char** pcur, flt* out) -> bool {
				using namespace n_parse;
				char* cur = *pcur;
	...
	static bool parse (std::string const& s, bool* out, int comp_count, mode_e mode) {
		return parse_vec(s, out, comp_count, n_parse::bool_);
	}

	static bool parse (type_e type, mode_e mode, std::string const& s, void* val) {
		switch (type) {
			case STRING	:	return parse(s, (std::string*)val);

			case INT_	:	return parse(s,   (int	*)val	 , 1, mode);
			case IV2	:	return parse(s, &((iv2	*)val)->x, 2, mode);
			case IV3	:	return parse(s, &((iv3	*)val)->x, 3, mode);
			case IV4	:	return parse(s, &((iv4	*)val)->x, 4, mode);

			case FLT	:	return parse(s,   (flt	*)val	 , 1, mode);
			case FV2	:	return parse(s, &((v2	*)val)->x, 2, mode);
			case FV3	:	return parse(s, &((v3	*)val)->x, 3, mode);
			case FV4	:	return parse(s, &((v4	*)val)->x, 4, mode);
			
			case BOOL	:	return parse(s,   (bool	*)val	 , 1, mode);
			case BV2	:	return parse(s, &((bv2	*)val)->x, 2, mode);
			case BV3	:	return parse(s, &((bv3	*)val)->x, 3, mode);
			case BV4	:	return parse(s, &((bv4	*)val)->x, 4, mode);
...
struct Node {
	...
	void update (void* val_, bool load, Value::type_e val_type, Value::mode_e val_mode) { // name and parent set externally
		if (load) {
			val.load(val_, val_type, val_mode);
		}
...
struct Save {
	...
	Node* _node (cstr name, void* val, Value::type_e val_type, Value::mode_e val_mode) {
		auto* n = get_node(name);
		n->update(val, trigger_load, val_type, val_mode);
		return n;
	}
	Node* _array_node (cstr name, int indx, void* val, Value::type_e val_type, Value::mode_e val_mode) {
	...
	void begin (cstr name) {
		auto* n = _node(name, nullptr, Value::STRUCT, Value::DIMENSIONLESS);
		cur_parent = n;
	}
	void end () {
		if (!cur_parent)
	...
	int begin_array (cstr name) {
		auto* n = _node(name, nullptr, Value::ARRAY, Value::DIMENSIONLESS);
		cur_parent = n;
	...
	bool to_xml () {
		...
		return write_text_file((filepath +".xml").c_str(), text);
	}
	...
	bool from_xml () {
		...
		std::string text;
		if (!load_text_file((filepath +".xml").c_str(), &text))
			return false;
		...
		assert(cur_parent == nullptr);
		root_children.clear(); // deletes all nodes
		
		if (xml_doc.first_node())
			_from_rapid_xml_recurse(xml_doc, cur_parent, &root_children);
